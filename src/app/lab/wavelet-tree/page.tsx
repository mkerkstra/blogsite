      ...
      <WaveletTree
        info={
          <>
            ...
            <p>
              To answer rank(c, i) &mdash; how many times does c appear up to position i &mdash;
              walk from root to leaf. At each node, count how many bits match c&apos;s side of the
              alphabet split up to the current position. That count becomes the new position in the
              child. When you reach the leaf for c, the position is the answer.
            </p>
            <p>
              ...
              library power compressed suffix arrays and FM-indexes used in bioinformatics tools
              like bowtie2 for genome alignment.
            </p>
            <p>
              The structure uses n log(sigma) bits total, matching the{" "}
              <Term id="zero-order-entropy">zero-order entropy</Term> of the string. Every query
              touches exactly one bitvector per level, so the depth of the tree directly determines
              the query cost. Pick a character and position, then watch the rank query propagate
              down.
            </p>
