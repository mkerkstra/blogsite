...
export default function XorFilterPage() {
  return (
    <>
      <LabHead slug="xor-filter" />
      <XorFilter
        info={
          <>
            <p>
              An XOR filter encodes set membership in a compact{" "}
              <Term id="fingerprint">fingerprint</Term>&nbsp;table. For each key, three hash
              functions pick three table positions. The table is constructed so that XORing the
              three values at those positions yields the key&apos;s fingerprint. Querying is three
              lookups and two XORs.
            </p>
            <p>
              Construction works by <Term id="peeling">peeling</Term> a 3-uniform{" "}
              <Term id="hypergraph">hypergraph</Term>. Each key is a hyperedge connecting its three
              hash positions. Find a position touched by exactly one key (degree 1), remove that
              key, repeat. If every key gets removed, fingerprints can be assigned in reverse order:
              each removed key&apos;s solo position is set to make the XOR equation hold. If peeling
              fails, rehash and retry.
            </p>
            <p>
              Graf and Lemire introduced XOR filters in 2020, showing they use about 1.23x the
              theoretical minimum space (roughly 9.84 bits per element for 8-bit fingerprints).
              Binary Fuse Filters (Dietzfelbinger and Walzer, 2022) improve on this with a tapered
              table shape that reduces construction failure probability to near zero while matching
              the same space efficiency.
            </p>
            <p>
